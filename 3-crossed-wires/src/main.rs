...
fn parse_sequence(sequence: &str) -> Vec<&str> {
  return sequence.split(',').collect();
}

fn trace_sequence(parsed_sequence: Vec<&str>) -> HashSet<(i32, i32)> {
  let mut moves: HashSet<(i32, i32)> = HashSet::new();
  let mut x: i32 = 0;
  let mut y: i32 = 0;

  for keyword in parsed_sequence {
    let direction = keyword.chars().next().unwrap();
    let amount: i32 = keyword[1..].parse().unwrap();
    for _i in 0..amount {
      if direction == 'U' {
        y = y + 1;
      } else if direction == 'D' {
...
fn trace_sequence_with_steps(parsed_sequence: Vec<&str>) -> HashMap<(i32, i32), i32> {
...
fn find_intersections(trace1: HashSet<(i32, i32)> , trace2: HashSet<(i32, i32)>) -> i32 {
  return trace1.intersection(&trace2)
    .map(|x| x.0.abs() + x.1.abs())
    .fold(10000, |highest, current| {
      if current < highest {
        return current
      } else {
        return highest
...
fn find_intersections_with_steps(trace1: HashMap<(i32, i32), i32> , trace2: HashMap<(i32, i32), i32>) -> i32 {
  let mut trace1Keys: HashSet<(i32, i32)> = HashSet::new();
  let mut trace2Keys: HashSet<(i32, i32)> = HashSet::new();

  for key in trace1.keys() {
    trace1Keys.insert(*key);
  }

  for key in trace2.keys() {
    trace2Keys.insert(*key);
  }

  return trace1Keys
    .intersection(&trace2Keys)
    .fold(1000000000, |highest, current| {
      let total = trace1.get(current).unwrap() + trace2.get(current).unwrap();
      if total < highest {
        return total
      } else {
        return highest
