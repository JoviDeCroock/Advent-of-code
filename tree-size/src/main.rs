...
fn build_tree(input: Vec<&str>) -> Rc<RefCell<Folder>> {
	let root_folder = Rc::new(RefCell::new(Folder{folders: Vec::new(), size: 0}));
		...
		match TerminalCommand::parse(line) {
			Some(TerminalCommand::CD(dir)) => {
				if dir == ".." {
					...
					let new = Rc::new(RefCell::new(Folder{folders: Vec::new(), size: 0}));
			...
			None => {
				...
				if parts[0].parse::<u32>().is_ok() {
					...
					let new = Rc::new(RefCell::new(Folder{folders: Vec::new(), size}));
					current_directory.borrow_mut().folders.push(new.clone());
				}
			}
		}
...
fn set_folder_sizes(folder: &mut Rc<RefCell<Folder>>) {
	...
	for mut child in &mut current_folder.folders {
		set_folder_sizes(&mut child);
...
fn sum_of_filtered_directories(folder: Rc<RefCell<Folder>>, max_size: u32) -> u32 {
	...
	if folder.borrow().size <= max_size && sub_folders.len() > 0 {
...
fn find_size_of_deleteable_folder(folder: Rc<RefCell<Folder>>, need_to_free: u32) -> u32 {
    ...
    let sub_folders = &folder.borrow().folders;
    let folder_size = folder.borrow().size.clone();
...
fn main() {
    ...
    println!("Solution 1: {} - Solution 2: {}", sum_of_filtered_directories(parent.clone(), 100000), find_size_of_deleteable_folder(parent.clone(), 30000000 - (70000000 - parent.borrow().size)));
}